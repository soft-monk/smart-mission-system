...
**二维地图能力的基座**：其他模块不需要了解 MapLibre——只看 [`doc/接口文档.md`](./doc/接口文档.md) 就能把地图用起来（注入数据、调用接口即可）。
MapLibre 渲染 + **API 驱动绘制**（无人机 / 区域 / 目标 / 链路 / 轨迹 / 扫描 / 标注…）。主系统「智能任务管理系统」通过别名 `@map2d` 消费它，用法与调用第三方库一致；本目录也可单独 clone、单独启动。

> 模块文档在 [`doc/`](./doc/)：
> [接口文档](./doc/接口文档.md)（**接入的唯一依据**：签名 / 参数 / 默认值 / 示例 / 错误约定）·
> [需求文档](./doc/需求文档.md)（定位与边界 / 需求条目 `M2-域-序号` / 验收 / 待确认）·
> [设计文档](./doc/设计文档.md)（分层 / 渲染管线 / 设计决策 / 已知边界）
...
│  ├─ 接口文档.md             #   ★ 接入的唯一依据（签名/参数/示例/错误约定）
│  ├─ 需求文档.md             #   定位与边界 / 需求条目（M2-域-序号）/ 验收 / 待确认
