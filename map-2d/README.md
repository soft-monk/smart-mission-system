...
>
> **想先看它现在能做什么？** 按第二节跑起来，右侧就是「功能验收台」——点按钮即可逐条验证
> （说明见 [doc/验收台使用说明.md](doc/验收台使用说明.md)）。
...
状态口径：**✅ 已完成 ｜ ⏳ 待完成**（当前 93 条需求中已完成 92 条，详见 [doc/需求文档.md](doc/需求文档.md) §0 完成情况总览）。
...
| **地图控件按需显示** | 指北针 / 鼠标位置经纬度 / 缩放按钮 / 比例尺**默认全部不显示**，调 `mapCommands.showControls([...])` 才出现（需求 M2-CTRL-01~05） | 独立宿主控制条上的「控件」勾选框 |
...
| **时间轴回放** | 模块给时间轴（播放/暂停/倍速/拖拽定位/自动跟随/采样点步进）与渲染，**数据与存储由宿主提供** | `<ReplayBar />` + `mapCommands.loadReplay(data)` |
| **圈层图元** | 距离环（等距圈）、方位线（射线）、方位圈（带刻度）、九宫格参考线 | `MapDraw.add('annulus', { kind: 'ring' \| 'bearing-line' \| 'bearing-ring' \| 'grid', ... })` |
| **目标聚合** | 密集目标按缩放聚合成计数气泡，放大自动散开（按屏幕像素聚类） | `mapCommands.setClusterOptions({ enabled: true })` |
| **标签避让与分级** | 重叠标签自动错让；图元可写 `minZoom` 决定从哪一级开始显示 | `MapDraw.add('label', { ..., minZoom: 12 })` |
| **国军标标绘符号** | 内置 12 个兵种符号，框形与颜色随敌我属性变化，支持旋转/缩放 | `MapDraw.add('symbol', { symbol: 'armor', affiliation: 'hostile', rotation: 30 })` |
| **样式模板** | 命名样式集中登记，图元引用名字；**改模板即批量生效** | `mapCommands.setStyleTemplates({...})` |
| **主题热切换** | 日间/夜间/高对比三套（底图参数 + 标签可读性），运行中切换 | `mapCommands.applyTheme('night')` |
| **显示模式** | 阶段自动推导 + 宿主手动覆盖（手动优先，换场景自动解除） | `mapCommands.setDisplayMode('任务规划')` |
| 接口文档 / 类型自带 / 模块开关集中 | 看文档即可接入 | [doc/接口文档.md](doc/接口文档.md) |

> **注**：控件默认不显示是模块的规矩；**宿主需要谁就自己开**。主系统在启动时调了一次
> `mapCommands.showControls(['compass','zoom','scale'])`，所以它的界面看起来和以前一样。
...
完整条目、验收标准与决策记录见 [doc/需求文档.md](doc/需求文档.md)。当前 **93 条需求中已完成 92 条**。

---

> **纯函数自测**：`node scripts/unit-test.mjs` —— 17 项断言（距离/面积/方位角/简化/抽稀/UTM），
> 零依赖、可直接跑。
